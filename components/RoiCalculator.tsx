...
const AED = (n: number) => n.toLocaleString("en-AE", { style: "currency", currency: "AED", maximumFractionDigits: 2 })
...
export default function RoiCalculator() {
  ...
  const onDownload = () => {
    downloadCSV(
      toCSV(
        calc.rows.map((r) => ({ date: r.dateISO, amount: r.amount, cumulative: r.cumulative, note: r.note ?? "" })),
      ),
      "mirfa-roi-schedule.csv",
    )
  }

  // ---- Chart config (responsive with viewBox) ----
  const W = 900
  const H = 320
  const PAD = 56
  const AXIS_PAD = 40
  ...
  // Scale for RETURNS (emphasize positive side):
  // Map cumulative from [-totalIn ... +net] to [0 .. totalOut] using (cum + totalIn) / totalOut
  ...
  const minAmountLabelGap = 60 // increased gap to prevent overlap
  const largestAmountsIdx = (() => {
    const arr = points
      .map((p, i) => ({ i, v: Math.abs(p.amount) }))
      .filter((x) => x.v > 1e-9)
      .sort((a, b) => b.v - a.v)
      .slice(0, 3) // reduced to 3 key amounts
      .map((x) => x.i)
  ...
  // Zero line (cumulative = 0)
  const zeroY = H - AXIS_PAD - PAD - ((0 + calc.totalIn) / denom) * (H - 2 * PAD - AXIS_PAD)

  const primaryColor = "hsl(var(--primary))" // Mirfa Gold
  const mutedColor = "hsl(var(--muted))"

  return (
    <Card className="rounded-2xl border shadow-sm p-4 sm:p-6 md:p-8">
      <style jsx>{`
        .brand-range {
        ...
        .brand-range::-webkit-slider-thumb {
          ...
          box-shadow: 0 2px 4px rgba(198, 161, 91, 0.3);
          cursor: pointer;
          margin-top: -6px;
        }
      ...
      <div className="flex flex-col gap-4 md:gap-6 md:flex-row md:items-end md:justify-between">
        <div className="flex-1 space-y-3">
          <label htmlFor="investment-slider" className="block text-sm font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">
        <Kpi label="XIRR" value={isFinite(calc.rate) ? `${(calc.rate * 100).toFixed(2)}%` : "—"} />
        <Kpi label="MOIC" value={isFinite(calc.moic) ? `${calc.moic.toFixed(2)}×` : "—"} />
        <Kpi label="Total Invested" value={AED(calc.totalIn)} />
        <Kpi label="Total Returned" value={AED(calc.totalOut)} />
        <Kpi label="Net Profit" value={AED(calc.net)} />
        ...
        <div className="w-full">
          ...
          >
            ...
            {points.length > 1 && (
              <>
                <path
                  d={`${d} L ${points[points.length - 1].x} ${H - AXIS_PAD} L ${points[0].x} ${H - AXIS_PAD} Z`}
                  fill="url(#fillArea)"
                />
                <path
                  d={d}
            ...
            {points.map((p, i) => (
              <g key={i}>
                ...
                {(i % dateStep === 0 || i === 0 || i === points.length - 1) && (
                  ...
                  >
                    {new Date(p.dateISO + "T00:00:00Z").toLocaleDateString("en-GB", {
                      month: "short",
                      year: "2-digit",
                    })}
                  </text>
                ...
                {amountLabelSet.has(i) && (
                  <text
                    x={p.x}
                    y={clampY(p.y + (p.amount > 0 ? -15 : 18) + (i % 3 === 0 ? -6 : i % 3 === 1 ? 0 : 6))}
                    textAnchor="middle"
            ...
            <rect
              ...
              rx={2}
            />
            <text x={PAD + 10} y={H - AXIS_PAD - 8} className="text-[10px] font-medium fill-primary">
              Investment Period Progress
            </text>
          </svg>
        </div>
      </div>
      ...
      <div className="mt-6 sm:mt-8">
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed text-sm min-w-[720px]">
            {/* set explicit column widths for consistent layout */}
            <colgroup>
              <col style={{ width: "16%" }} />
              <col style={{ width: "22%" }} />
              <col style={{ width: "22%" }} />
              ...
              {calc.rows.map((r, i) => (
                <tr key={i} className={i % 2 === 0 ? "bg-muted/30" : ""}>
                  <td className="py-2 px-3 font-mono text-xs whitespace-nowrap">
                    {new Date(r.dateISO + "T00:00:00Z").toLocaleDateString("en-GB", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
                  ...
                  <td className="py-2 px-3 text-right font-mono tabular-nums whitespace-nowrap">
                    {Math.abs(r.amount) < 1e-9 ? (
                      "—"
                    ) : (
                      <span className={r.amount > 0 ? "text-green-600" : "text-red-600"}>{AED(r.amount)}</span>
                    )}
                  </td>
                  <td className="py-2 px-3 text-right font-mono tabular-nums whitespace-nowrap">
